...
public class DBValidatorEMEA extends OTPValidator implements ACS {
	...
	public static COResponse checkExpiryOfOTP(ACSRequest request, COConfig config, long otpGenerationTime, HashMap<String, Object> additionalCustomerData){
		...
		if (result > longConfigOtpGenerationTime){
			...
			// otp is still valid
			logger.info("OTP didnt time out");
	...
	public static COResponse actionOnOTPTimeOut(ACSRequest request, COConfig config, HashMap<String, Object> additionalCustomerData) {
		...
		if (config.getValue(OTP_TIMEOUT_ACTION) != null){
			logger.info(request, "OTP_TIMEOUT_ACTION config has been Set.EMEA.");
			coResponse = COUtil.createCOResponse(false, 0, OTPTIMEOUT, OTPTIMEOUT, GenericOTPHandlerEMEA.retrieveACSResponse(config.getValue(OTP_TIMEOUT_ACTION)),
		 ...
		 additionalCustomerData.put(GenericOTPHandler.error_msg, OTPFAILED);
			...
			if (!Boolean.valueOf(config.getValue(NUM_OF_RESENDS))){
				if (updateResendCounter >= (config.getIntValue(NUM_OF_RESENDS))){
					additionalCustomerData.put(GenericOTPHandler.error_msg, additionalCustomerData.get(GenericOTPHandler.error_msg) != null?
							additionalCustomerData.get(GenericOTPHandler.error_msg) + "," +
							GenericOTPHandler.RESENTEXCEEDED: GenericOTPHandler.RESENTEXCEEDED);
				}
			}
